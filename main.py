...
from taming.models.vqgan import (
    GumbelVQ,
    VQModel,
    get_obj_from_str,
...
class EpochModelCheckpoint(ModelCheckpoint):
    ...
    def _save_last_checkpoint(
        self, trainer, pl_module, epoch, ckpt_name_metrics, filepath
    ):
    ...
    try:
        ...
        if opt.resume:
            if config.model.target == "taming.models.vqgan.VQModel":
                model = VQModel(**config.model.params, ckpt_path=ckpt)
                # model.eval()
            elif (
                config.model.target
                ...
                model = GumbelVQ(**config.model.params, ckpt_path=ckpt)
                # model.eval()
            else:
        ...
        if opt.every_n_epochs:
            trainer_kwargs["checkpoint_callback"] = EpochModelCheckpoint(
                dirpath=ckptdir,
                filename="{epoch:06}",
                verbose=True,
                save_last=True,
